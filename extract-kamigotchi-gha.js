...
async function versionFile(key) {
  try {
    ...
    console.warn(
      `   ⚠️  Could not check ${key} before versioning: ${err.message}`,
    );
    return;
  }
  ...
  try {
    ...
    parallelUploads3.on("httpUploadProgress", (progress) => {
      if (progress.loaded % (1024 * 1024 * 100) < 1024 * 1024 * 15) {
        console.log(
          `   Progress: ${Math.round(progress.loaded / 1024 / 1024)}MB uploaded`,
        );
      }
    });
...
async function resolveWildOwners(wildIds, accountHexMap) {
  // accountHexMap: { [hexAddr]: [accountName, accountIndex] } — built server-side in page.evaluate
  // Both name and index are already correct; no reverse scan needed.
  ...
  for (let i = 0; i < wildIds.length; i += WILD_RPC_BATCH) {
      ...
      batch.map(async (kamiIndex) => {
        const response = await fetch(WILD_RPC_URL, {
          ...
          body: JSON.stringify({
            ...
            method: "eth_call",
            params: [
              { to: WILD_NFT_CONTRACT, data: encodeOwnerOfCalldata(kamiIndex) },
              "latest",
            ],
          }),
        });
        ...
        const acc = accountHexMap[addr];
        return {
          kamiIndex,
          accountIndex: acc ? acc[1] : null,
          accountName: acc ? acc[0] : null,
        };
      }),
    );
    ...
    results.forEach((r, j) => {
      if (r.status === "fulfilled") {
        ...
        console.warn(
          `   ⚠️  ownerOf failed for #${batch[j]}: ${r.reason?.message}`,
        );
      }
    });
  }
  ...
  console.log(
    `   ✅ Resolved ${resolved}/${wildIds.length} wild owners by name`,
  );
...
async function runExtraction() {
  ...
  try {
    ...
    while (retries > 0 && !dataLoaded) {
      const testResult = await page.evaluate(() => {
        ...
        return {
          ...
          maxId:
            allKami?.length > 0 ? Math.max(...allKami.map((k) => k.index)) : 0,
        };
      });
      console.log(
        `   📈 Progress: ${testResult.count} items | Max ID: ${testResult.maxId}`,
      );
      if (testResult.count > 0 && testResult.count === previousCount) {
        dataLoaded = true;
        break;
      }
    ...
    if (!dataLoaded || previousCount === 0) {
      throw new Error(
        `Data load failed: received 0 items after all retries. The site may be down or the network API did not load.`,
      );
    }
    ...
    const {
      ...
      const allFull = network.explorer.kamis.all({
        traits: true,
        stats: true,
        progress: true,
      });
      ...
      allFull.forEach((k) => {
        ...
        kamiInfo[k.index] = {
          ...
          stats: k.stats
            ? STAT_KEYS.map((s) => k.stats[s]?.total ?? null)
            : null,
        };
      });
      ...
      // Accounts: build three compact structures server-side to avoid sending kamis[] arrays to the client.
      // kamiOwnerMap:  { [kamiIndex]: [accountName, accountIndex] } — pre-inverted, replaces kamis[] loop client-side
      // accountIdMap:  { [id]: accountName } — for trade history seller/buyer resolution
      // accountHexMap: { [hexAddr]: [accountName, accountIndex] } — for wild owner RPC resolution
      ...
      allAccounts.forEach((acc) => {
        ...
        const idx = String(acc.index);
        const id = acc.id ? BigInt(acc.id).toString() : null;
        if (id && name) {
          accountIdMap[id] = name;
          const hexAddr = "0x" + BigInt(id).toString(16).toLowerCase();
          accountHexMap[hexAddr] = [name, idx];
        }
        if (Array.isArray(acc.kamis)) {
          acc.kamis.forEach((k) => {
            kamiOwnerMap[k.index] = [name, idx];
          });
        }
      });
      ...
      const prices = (() => {
        try {
          ...
          return { mintPrice: null, rerollPrice: null };
        }
      })();
      ...
      try {
        ...
        const rarityMap = {};
        const overallMap = {};
        rarityResults.forEach((r) => {
          rarityMap[r.index] = r.rarity;
        });
        overallResults.forEach((r) => {
          overallMap[r.index] = r.score;
        });
        allIds.forEach((id) => {
    ...
    await Promise.all([uploadBundleToR2(bundle), uploadMetaToR2(slimMeta)]);

    console.log("\n" + "=".repeat(60));
    console.log(
      `✅ COMPLETE: ${bundle.kamiMetadata.totalCount} items | ${bundle.kamiMetadata.extractionDuration}s`,
    );
    console.log("=".repeat(60));
  } catch (error) {
